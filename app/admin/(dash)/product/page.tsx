...
// Shallow scan of /public for pickable media (one level deep into folders).
...
export default async function AdminProductPage() {
  const [product, assets] = await Promise.all([
    getProductContent(),
    Promise.resolve(listPublicAssets()),
  ]);
    ...
    <div className="flex flex-col gap-[clamp(20px,4vw,32px)]">
      <div className="flex flex-col gap-[6px]">
        ...
        <p className="m-0 max-w-[60ch] text-[14px] text-ink-soft">
          Changes go live on the site immediately after saving. Prices are charged
          through Stripe exactly as set here.
        </p>
      </div>

      <ProductForm initial={product} assets={assets} />
    </div>
  );
}
